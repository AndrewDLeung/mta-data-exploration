...
Now let's join the new `remote_booth_station` with the `mta_locations` data frame:
...
station_join <- inner_join(remote_booth_station,
                           mta_locations,
                           by = "complex_id")

tud19_join <- inner_join(tud19_sum,
                         station_join,
                         by = "unit") 

tud19_join <- tud19_join %>% 
  distinct(turnstile_id, date,
           .keep_all = TRUE) %>% 
  arrange(turnstile_id)
```
