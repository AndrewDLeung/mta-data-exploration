...
complex_list <- unique(tud19_location_join[, c("station_abbr", 
                                               "station_long", 
                                               "complex_id")])
```
...
system_weekly_2019 <- tud19_location_join %>% 
  mutate(week_num = strftime(date, format = "%V")) %>% 
  group_by(week_num) %>% 
  summarise(weekly_entries = sum(entry_total),
            weekly_exits = sum(exit_total))
...
Now, let's repeat this cleaning process for the 2020 and 2021 data. 
